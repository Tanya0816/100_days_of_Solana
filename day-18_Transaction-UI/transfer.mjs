...
async function waitForCommitment(rpc, signature, targetCommitment) {
    ...
    while (true) {
        ...
        if (status?.err) {
            throw new Error(`Transaction failed on-chain: ${JSON.stringyfy(status.err)}`);
        }
...
async function transferWithConfirmation(rpc, signer, toAddress, amountInSOL) {
    ...
    //Step B : Wait for confirmend status
...
async function main() {
    ...
    try {
        ...
        console.log(`https://explorer.solana.com/txx/${signature}?cluster=devnet`);
    } catch (error) {
        console.error("\nTransaction failed");
